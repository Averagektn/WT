...
-- Schema mycoolstore
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `mycoolstore` DEFAULT CHARACTER SET utf8mb3 ;
USE `mycoolstore` ;
...
CREATE TABLE IF NOT EXISTS `mycoolstore`.`film` (
  ...
  PRIMARY KEY (`flm_id`),
  UNIQUE INDEX `flm_name_UNIQUE` (`flm_name` ASC) VISIBLE)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `mycoolstore`.`film_media` (
  ...
  CONSTRAINT `fk_film_media_film1`
    FOREIGN KEY (`fm_id`)
    REFERENCES `mycoolstore`.`film` (`flm_media`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
